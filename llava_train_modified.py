...
def find_all_linear_names(model):
    ...
    return list(lora_module_names) + ["lm_head", "embed_tokens"]
...
def train(attn_implementation=None):
    ...
    if training_args.lora_enable:
        ...
        if training_args.local_rank == 0 or training_args.local_rank == -1:
            ...
            model.save_pretrained(training_args.output_dir + "/adapter", state_dict=state_dict)
            tokenizer.save_pretrained(training_args.output_dir)
            torch.save(non_lora_state_dict, os.path.join(training_args.output_dir, "pytorch_model.bin"))
    else:
